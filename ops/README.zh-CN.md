...
已发现上游 `train.py` 将实际 `envs/my_dmc.py` 写成 `env.my_dmc`；这会影响训练入口导入。后续需要单独记录修复，不将其归为模型创新。

Drama 包含自己的 `mamba_ssm/` Python 源码，其中有 `MambaWrapperModel` 等定制接口。因此验收必须从仓库根目录执行，核对模块来源；仅成功导入 PyPI 的 Mamba 不够。

## 验收范围

环境验收通过只说明被检查的计算路径可用，不代表完整训练协议、CUDA Graph、torch.compile 或研究假说已验证。正式训练另行确定配置和预算。
